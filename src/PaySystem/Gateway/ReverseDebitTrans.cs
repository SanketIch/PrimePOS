...
{
    /// <summary>
    /// Handles the properties and Methords that process a credit or void of a prevous Debit Card Transaction
    /// NOTE: A previous Transaction is required
    /// </summary>
    public class ReverseDebitTrans : IDisposable
    {
       // private PrismPay.DebitReturn oDebitReturn;
        //private TransactionSOAPBindingImplClient OGatewayClient; 
        ...
        public ReverseDebitTrans()
        {
           // oDebitReturn = new PrismPay.DebitReturn();
           // oDebitReturn.acctid = "PYL2F";
        }
        ...
        /// <summary>
        /// Void a Previous Debit Card Transaction(A previous transaction is required )
        /// </summary>
        ...
        public void ReturnDebitTrans(out TransactionResult oresult, out Exception ex)
        {
            ex = null;
            ...
            switch (GatewayManager.SelectedGateway)
            {
                case Gateway.PrismPay:
                    PP_ReturnDebitTrans(out oresult, out ex);
                    break;
                case Gateway.WorldPay:
                    WP_ReturnDebitTrans(out oresult, out ex);
                    break;
                default:
        ...
        /// <summary>
        /// Issue Credit Previous Debit Card Transaction(A previous transaction is required )
        /// </summary>
        /// <param name="oresult">Returns the Transaction result as an instance of the TransactionResult class</param>
        /// <param name="ex">Returns the Exception(if any) that Occured During the Transaction or null </param>
        public void VoidDebitTrans(out TransactionResult oresult, out Exception ex)
        {
            ex = null;
            oresult = new TransactionResult();
            ...
            {
                case Gateway.PrismPay:
                    PP_VoidDebitTrans(out oresult, out ex);
                    break;
                case Gateway.WorldPay:
                    WP_VoidDebitTrans(out oresult, out ex);
                    break;
                default:
                    ex = new Exception("Gateway not selected");
                    break;
        ...
        private void PP_VoidDebitTrans(out TransactionResult oResult, out Exception oEx)
        {
            oResult = new TransactionResult();
            oEx = null;
            try
            {
                PrismPay.DebitReturn oDebitReturn = new PrismPay.DebitReturn();
                oDebitReturn.acctid = GatewayManager.AccountId;
                oDebitReturn.amount = _Amount;
                oDebitReturn.historyid = _HistoryId;
                oDebitReturn.orderid = _OrderId;

                if (string.IsNullOrEmpty(GatewayManager.MerchantPin))
                {
                    if (!string.IsNullOrEmpty(_MerchantPin))
                    {
                        oDebitReturn.merchantpin = _MerchantPin;
                    }
                }
                else
                    oDebitReturn.merchantpin = GatewayManager.MerchantPin;


                if (!string.IsNullOrEmpty(GatewayManager.SubId))
                {
                    oDebitReturn.subid = GatewayManager.SubId;
                }
                ...
                oProcessResult = oGatewayService.processDebitVoid(oDebitReturn);
        ...
        private void WP_VoidDebitTrans(out TransactionResult oResult, out Exception oEx)
        {
            oResult = new TransactionResult();
            oEx = null;
            try
            {
                WorldPay.DebitReturn oDebitReturn = new WorldPay.DebitReturn();
                oDebitReturn.acctid = GatewayManager.AccountId;
                oDebitReturn.amount = _Amount;
                oDebitReturn.historyid = _HistoryId;
                oDebitReturn.orderid = _OrderId;
        ...
        private void PP_ReturnDebitTrans(out TransactionResult oResult, out Exception oEx)
        {
            oResult = new TransactionResult();
            oEx = null;
            ...
            try
            {
                ...
                oProcessResult = oGatewayService.processDebitReturn(oDebitReturn);
                oResult.LoadResultData(oProcessResult);
            }
            catch (Exception e)
            {
        ...
        private void WP_ReturnDebitTrans(out TransactionResult oResult, out Exception oEx)
        {
            oResult = new TransactionResult();
            oEx = null;
