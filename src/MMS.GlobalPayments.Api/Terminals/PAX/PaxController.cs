...
using NLog; //NG SDKUPDATE 20/9/2022
using System;
    ...
    public class PaxController : DeviceController {
        ...
        internal override IDeviceCommInterface ConfigureConnector() {
            switch (_settings.ConnectionMode) {
                case ConnectionModes.TCP_IP: //NG SDKUPDATE 20/9/2022
                ...
                case ConnectionModes.HTTP: //NG SDKUPDATE 20/9/2022
                    logger.Debug("PaxDevice.HTTP : ConnectionMode=" + _settings.ConnectionMode + "; IPAddress= " + _settings.IpAddress + "; Port = " + _settings.Port + "; Timeout=" + _settings.Timeout);
        ...
        internal override ITerminalResponse ProcessTransaction(TerminalAuthBuilder builder) {
            ...
            switch (builder.PaymentMethodType) {
                case PaymentMethodType.Credit:
                    return (ITerminalResponse)DoCredit(request);
                case PaymentMethodType.Debit:
                    return (ITerminalResponse)DoDebit(request);
                case PaymentMethodType.Gift:
                    return (ITerminalResponse)DoGift(request);
                case PaymentMethodType.EBT:
                    return (ITerminalResponse)DoEBT(request);
                default:
